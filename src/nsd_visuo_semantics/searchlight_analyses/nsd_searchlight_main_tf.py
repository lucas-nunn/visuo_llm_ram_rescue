...
from nsd_visuo_semantics.searchlight_analyses.tf_searchlight import tf_searchlight_corr
...
def nsd_searchlight_main_tf(model_name, rdm_distance,
                            ...
                            OVERWRITE, subject=1, n_sessions=20, max_samples=None):
    ...
    # optional cap for quick tests (the saved sampling pool is still computed over all samples)
    n_samples_to_run = subj_n_samples if max_samples is None else min(subj_n_samples, max_samples)
    ...
    for j in range(n_samples_to_run):
        ...
        if os.path.exists(file_save) and not OVERWRITE:
            ...
            betas_sampled = betas_sampled.astype(np.float32, copy=False)
            ...
            # Compute each batch of brain RDMs and correlate it immediately.
            # This avoids materialising the multi-GB full-brain RDM matrix.
            print("computing searchlight RDM correlations")
            brain_maps = tf_searchlight_corr(
                betas_sampled,
                all_indices,
                sorted_indices,
                model_rdms_sample,
